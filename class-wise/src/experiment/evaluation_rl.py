...
from src.data.imagenet import get_train_loader, get_val_loader, get_relabel_loader

from src.algorithm.svc_mia import SVC_MIA
from src.algorithm.sign_sgd import SignSGD
from src.algorithm.l1_regularization import L1_regularization
...
Section('train.optimizer').params(
    type=Param(OneOf(['SignSGD', 'AdamW', 'SGD']), default='SGD'),
...
class Trainer:
    ...
    def create_train_loader(self, path, num_workers, in_memory, batch_size, distributed, indices=None):
        ...
        self.forget_loader, self.decoder = get_relabel_loader(path, num_workers, in_memory, batch_size, res, self.device,
    ...
    @param('logging.path')
    @param('logging.model_path')
    @param('logging.save_intermediate_frequency')
    def run(self, epoch, alpha, dry_run, path=None, model_path=None, save_intermediate_frequency=None):
    ...
    def train_loop(self, epoch):
        ...
        def train_on_loader(loader):
            iterator = tqdm(loader, ncols=160) if self.gpu_id == 0 else loader
            for images, target, _ in iterator:
                ...
                with autocast():
                    ...
                    loss_train = self.loss(output, target) + 1e-4 * L1_regularization(self.network)
        ...
        stats = train_on_loader(self.forget_loader)
        stats = train_on_loader(self.remain_loader)
        return stats
