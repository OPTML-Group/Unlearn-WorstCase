...
from src.algorithm.svc_mia import SVC_MIA
...
class Trainer:
    ...
    def create_train_loader(self, path, num_workers, in_memory, batch_size, distributed, indices=None):
        ...
        self.forget_loader, self.decoder = get_train_loader(path, num_workers, in_memory, batch_size, res, self.device,
                                                           indices, distributed = distributed)
        
        # Assuming all_indices is a tensor or list of all possible indices
        all_indices = torch.arange(0, 1281167)

        # Find the complement of the forget_loader indices
        remain_indices = torch.tensor(list(set(all_indices.tolist()) - set(indices.tolist())))
        self.remain_loader, _ = get_train_loader(path, num_workers, in_memory, batch_size, res, self.device, remain_indices, distributed=distributed)
    ...
    def create_val_loader(self, path, num_workers, batch_size, resolution, distributed, indices=None):
        ...
        all_indices = torch.arange(0, 50000)
        remain_indices = torch.tensor(list(set(all_indices.tolist()) - set(indices.tolist())))
        self.val_loader = get_val_loader(path, num_workers, batch_size, resolution, self.device, remain_indices, 
                                         distributed=distributed)

    @param('train.epoch')
    def get_cosine_scheduler(self, epoch):
        return torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=epoch * len(self.remain_loader))
    ...
    def get_cosine_with_warmup_scheduler(self, epoch, warmup_epoch, starting_factor):
        scheduler = torch.optim.lr_scheduler.ChainedScheduler([
                torch.optim.lr_scheduler.LinearLR(self.optimizer, start_factor=starting_factor, total_iters=warmup_epoch * len(self.remain_loader)),
                torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=(epoch - warmup_epoch) * len(self.remain_loader))
            ])
        return scheduler
    ...
    def get_step_scheduler(self, step_ratio, step_size):
        return torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=step_size * len(self.remain_loader),
                                               gamma=step_ratio)
    ...
    def get_cyclic_scheduler(self, epoch, lr_peak_epoch):
        return torch.optim.lr_scheduler.CyclicLR(
                                                ...
                                                step_size_up=lr_peak_epoch * len(self.remain_loader),
                                                step_size_down=(epoch - lr_peak_epoch) * len(self.remain_loader),
    ...
    def run(self, epoch, dry_run, path=None, save_intermediate_frequency=None):
        ...
        forget_accuracy = self.accuracy(self.forget_loader)
        remain_accuracy = self.accuracy(self.remain_loader)
        test_accuracy = self.accuracy(self.val_loader)
        ...
        mia_stats = self.forget_efficacy()
    ...
    def train_loop(self, epoch):
        ...
        iterator = tqdm(self.remain_loader, ncols=160) if self.gpu_id == 0 else self.remain_loader
    ...
    def accuracy(self, loader):
        self.network.eval()
        iterator = tqdm(loader, ncols=120) if self.gpu_id == 0 else loader
    ...
    @param('dataset.train_path', 'train_path')
    @param('dataset.val_path', 'val_path')
    @param('dataset.num_workers')
    @param('validation.batch_size')
    @param('validation.resolution')
    @param('train.distributed')
    @param('dataset.indices.training', 'train_indices')
    @param('dataset.indices.testing', 'test_indices')
    def forget_efficacy(self, train_path, val_path, num_workers, batch_size, 
                          resolution, distributed, train_indices=None, test_indices=None):
        
        if train_indices is not None:
            train_indices = torch.load(train_indices, map_location="cpu")

        all_indices = torch.arange(0, 1281167)
        # Find the complement of the forget_loader indices
        remain_indices = torch.tensor(list(set(all_indices.tolist()) - set(train_indices.tolist())))
        sub_remain_indices = remain_indices[:10000]

        forget_loader = get_val_loader(train_path, num_workers, batch_size, resolution, self.device, train_indices, 
                                    distributed=distributed)     
        remain_loader = get_val_loader(train_path, num_workers, batch_size, resolution, self.device, sub_remain_indices, 
                                    distributed=distributed)

        if test_indices is not None:
            test_indices = torch.load(test_indices, map_location="cpu")
        
        all_indices = torch.arange(0, 50000)
        remain_indices = torch.tensor(list(set(all_indices.tolist()) - set(test_indices.tolist())))

        sub_remain_indices = remain_indices[:10000]
        val_loader = get_val_loader(val_path, num_workers, batch_size, resolution, self.device, remain_indices, 
                                         distributed=distributed)
        
        stats = SVC_MIA(
            shadow_train=remain_loader,
