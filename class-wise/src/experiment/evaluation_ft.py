...
Section('train').params(
    ...
    alpha=Param(float, 'l1 regularization parameter', required=True),
...
class Trainer:
    ...
    def run(self, epoch, alpha, dry_run, path=None, model_path=None, save_intermediate_frequency=None):
        ...
        for e in range(self.start_epoch, epoch):
            ...
            current_alpha = alpha * (1 - e / epoch)
            print(current_alpha)
            self.decoder.output_size = (res, res)
            train_stats = self.train_loop(e, current_alpha)
    ...
    def train_loop(self, epoch, alpha):
        self.network.train()
        ...
        for images, target, _ in iterator:
            ...
            with autocast():
                ...
                loss_train = self.loss(output, target) + alpha * L1_regularization(self.network)
