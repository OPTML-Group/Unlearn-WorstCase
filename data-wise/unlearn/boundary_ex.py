...
@w_iterative_unlearn
def w_boundary_expanding_iter(
    data_loaders, model, criterion, optimizer, epoch, args, w=None, mask=None
):
    train_loader = data_loaders["forget"]
    ...
    for i, (image, target, index) in enumerate(train_loader):
        ...
        print(output_clean)
        loss = torch.sum(w[index] * criterion(output_clean, target_label))
        print(loss)
        optimizer.zero_grad()
        loss.backward()
...
def w_boundary_expanding(data_loaders, model, criterion, args, w=None, mask=None):
    expand_model(model)
    return w_boundary_expanding_iter(data_loaders, model, criterion, args, w, mask)